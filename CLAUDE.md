...
- `cli/` — `subc`: login/logout/whoami, secure mbta-style profiles, and `ol-runbook` integrations for Claude Code, Codex, OpenCode, Cursor, Copilot, and Pi. `subc login` creates the default profile; `subc setup` configures all six integrations at once.
