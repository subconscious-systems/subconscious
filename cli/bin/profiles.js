...
API_KEY={API_KEY}
# Run \`subc models\` to see the gateway's current model catalog.
MODEL={MODEL}
...
export async function modelsCommand(profile, options = {}) {
  const gatewayUrl =
    process.env.SUBCONSCIOUS_BASE_URL?.trim() ||
    profile?.values?.GATEWAY_URL?.trim() ||
    DEFAULT_GATEWAY_URL;
  const configuredModel =
    process.env.SUBCONSCIOUS_MODEL?.trim() || profile?.values?.MODEL?.trim() || DEFAULT_MODEL;
  const catalog = await resolveGatewayModels(gatewayUrl, options);

  console.log(`\n  ${c.bold}Available models${c.reset}\n`);
  for (const model of catalog.models) {
    const suffix = model === configuredModel ? ` ${c.dim}(profile default)${c.reset}` : '';
    console.log(`  ${c.cyan}${model}${c.reset}${suffix}`);
  }
  if (catalog.source === 'fallback') {
    console.error(
      `\n  ${c.yellow}Could not reach ${gatewayUrl}/v1/models; showing the bundled offline fallback.${c.reset}`,
    );
    console.error(`  ${c.dim}${catalog.error.message}${c.reset}`);
  } else if (!catalog.models.includes(configuredModel)) {
    console.error(
      `\n  ${c.yellow}Profile default ${configuredModel} is not currently advertised by this gateway.${c.reset}`,
    );
  }
  console.log();
}
