...
test('parseClaudeVersion reads the official Claude Code banner', () => {
  ...
  assert.equal(
    parseClaudeVersion('@anthropic-ai/claude-code/2.1.180'),
    '2.1.180',
  );
...
test('claudeVersionNeedsUpgrade treats the model-picker floor as inclusive', () => {
  assert.equal(
    claudeVersionNeedsUpgrade('2.1.241', MIN_CLAUDE_CODE_VERSION),
    true,
  );
  assert.equal(
    claudeVersionNeedsUpgrade('2.1.242', MIN_CLAUDE_CODE_VERSION),
    false,
  );
  assert.equal(
    claudeVersionNeedsUpgrade('2.1.259', MIN_CLAUDE_CODE_VERSION),
    false,
  );
