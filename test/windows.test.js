...
import {
  ...
  windowsInvocation,
  windowsPath,
} from '../bin/windows/process.js';
import {
  mergeHooks,
  windowsHookCommand,
  windowsSetup,
} from '../bin/windows/setup.js';
...
test('Windows paths cover user, npm and native-installer system profile locations', () => {
  ...
  assert.ok(
    dirs.includes('C:\\WINDOWS\\System32\\config\\systemprofile\\.local\\bin'),
  );
  assert.ok(dirs.includes('C:\\Users\\Admin\\AppData\\Roaming\\npm'));
  assert.equal(
    windowsPath(['C:\\Bin', 'c:\\bin'], 'C:\\Other;"c:\\BIN"'),
    'C:\\Bin;C:\\Other',
  );
  assert.deepEqual(
    windowsEnv(
      { Path: 'old', SystemRoot: 'C:\\Windows' },
      { PATH: 'new', nothing: undefined },
    ),
    { PATH: 'new', SYSTEMROOT: 'C:\\Windows' },
  );
});
...
test('Windows executable lookup honors preferred dirs and prefers native executables', () => {
  ...
  assert.equal(
    resolveWindowsExecutable('claude', options),
    'C:\\native\\claude.exe',
  );
...
test('npm shims preserve exact JSON, Unicode, empty arguments and shell metacharacters', async (t) => {
  ...
  const invocation = windowsInvocation(shim, args, process.env, {
    pathApi: path,
    resolve: () => shim,
  });
  assert.equal(invocation.command, process.execPath);
  const result = await child(invocation.command, invocation.args, {
    env: invocation.env,
  });
...
for (const name of ['npm', 'npx'])
  test(`Node's bundled ${name}.cmd resolves the CLI, not npm-prefix.js`, async (t) => {
    const dir = await temporary(t);
    const bin = path.join(dir, 'node_modules', 'npm', 'bin');
    await fs.mkdir(bin, { recursive: true });
    for (const file of ['npm-prefix.js', 'npm-cli.js', 'npx-cli.js'])
      await fs.writeFile(path.join(bin, file), '// fixture');
  ...
  assert.throws(
    () =>
      windowsInvocation(
        ...
        [],
        {},
        { resolve: () => 'C:\\custom.cmd', readFile: () => 'echo %*' },
      ),
    /custom batch wrapper/,
  );
});
...
test('Windows editors preserve paths and wait for VS Code', () => {
  assert.deepEqual(windowsEditor(undefined, {}), {
    command: 'notepad.exe',
    args: [],
  });
  assert.deepEqual(
    windowsEditor(undefined, {
      VISUAL: '"C:\\Program Files\\Code\\code.cmd" --wait',
    }),
    { command: 'C:\\Program Files\\Code\\code.cmd', args: ['--wait'] },
  );
  assert.deepEqual(windowsEditor('code', {}), {
    command: 'code',
    args: ['--wait'],
  });
...
test('PowerShell dynamic data stays in environment rather than executable source', () => {
  const spec = powershellCommand(
    'Start-Process -FilePath $env:SUBC_BROWSER_URL',
    { SUBC_BROWSER_URL: 'https://example.test/?a=1&b=%PATH%' },
  );
...
test('native child status is preserved and signal listeners are removed', async () => {
  ...
  assert.equal(
    await runWindows(process.execPath, ['-e', 'process.exit(7)'], {
      stdio: 'ignore',
    }),
    7,
  );
  ...
  await assert.rejects(
    runWindows('subc-definitely-missing-executable', []),
    /Could not find/,
  );
  assert.equal(process.listenerCount('SIGINT'), before);
});
...
test('Windows Claude launch retains model, subagent, telemetry and picker settings', async () => {
  ...
  assert.equal(
    launch.env.OTEL_EXPORTER_OTLP_LOGS_ENDPOINT,
    'https://gateway.example/v1/logs',
  );
  const equals = await windowsLaunch(
    'claude-code',
    [
      '--api-key=override',
      '--compact-window=123456',
      '--',
      '--model',
      'agent-model',
    ],
...
test('Codex uses a temporary catalog, native argv and opt-in external tools/subagents', async (t) => {
  ...
  const spec = await windowsLaunch(
    ...
    {
      ...env,
      CODEX_AUTO_COMPACT_TOKEN_LIMIT: '1800000',
  ...
  const catalogPath = JSON.parse(
    spec.args
      .find((arg) => arg.startsWith('model_catalog_json='))
      .split('=')
      .slice(1)
      .join('='),
  );
  ...
  const external = await windowsLaunch('codex', ['--external-tools'], env, {
    tempRoot: root,
  });
  assert.ok(!external.args.includes('features.apps=false'));
  await assert.rejects(
    executeWindowsLaunch(external, async () => {
...
test('invalid Codex flags cannot leave temp files', async (t) => {
  const root = await temporary(t);
  await assert.rejects(
    windowsLaunch('codex', ['--context-window', '-5'], env, { tempRoot: root }),
    /positive integer/,
  );
  await assert.rejects(
    windowsLaunch('codex', ['--reasoning-effort', 'impossible'], env, {
      tempRoot: root,
    }),
    /reasoning-effort/,
  );
  assert.deepEqual(await fs.readdir(root), []);
  assert.throws(
    () => parseOptions(['--subagents=false'], { '--subagents': true }),
    /does not take a value/,
  );
});

for (const selectedVision of [true, false])
  test(`Windows harness vision metadata works with DeepSeek V4.1 ${selectedVision ? 'selected' : 'in the picker'}`, async (t) => {
    const root = await temporary(t);
    ...
    const environment = {
      ...env,
      MODEL: selectedVision ? visionModel : textModel,
    ...
    for (const [id, model] of Object.entries(
      ocConfig.provider.subconscious.models,
    )) {
    ...
    const codex = await windowsLaunch('codex', [], environment, {
      tempRoot: root,
    });
    ...
    const catalogPath = JSON.parse(
      codex.args
        .find((arg) => arg.startsWith('model_catalog_json='))
        .slice('model_catalog_json='.length),
    );
    const catalog = JSON.parse(await fs.readFile(catalogPath, 'utf8'));
    ...
    const dsh = await windowsLaunch('deepseek-harness', [], environment, {
      tempRoot: root,
    });
    t.after(dsh.cleanup);
    ...
    for (const client of ['pi', 'copilot']) {
      ...
      for (const model of provider.models) {
        if (client === 'pi')
          assert.deepEqual(
            model.input,
            model.id === visionModel ? ['text', 'image'] : undefined,
            model.id,
          );
...
test('OpenCode, Pi and sc have independent native launch specifications', async () => {
  const oc = await windowsLaunch('opencode', ['--', '--continue'], {
    ...
    OPENCODE_OUTPUT_LIMIT: '7890',
  });
  const config = JSON.parse(oc.env.OPENCODE_CONFIG_CONTENT);
  assert.equal(
    config.provider.subconscious.options.baseURL,
    'https://gateway.example/v1',
  );
  assert.equal(
    config.provider.subconscious.options.apiKey,
    '{env:SUBCONSCIOUS_API_KEY}',
  );
  assert.equal(
    config.provider.subconscious.options.modelsDiscovery.enabled,
    false,
  );
  assert.deepEqual(Object.keys(config.provider.subconscious.models), [
    env.MODEL,
    'custom/model',
  ]);
  assert.ok(!oc.env.OPENCODE_CONFIG_CONTENT.includes('sk-test'));
  assert.deepEqual(config.provider.subconscious.models[env.MODEL].limit, {
    context: 123456,
    output: 7890,
  });
  assert.deepEqual(oc.args, ['--continue']);
  assert.deepEqual((await windowsLaunch('pi', ['--continue'], env)).args, [
    '--provider',
    'subconscious',
    '--model',
    env.MODEL,
...
test('DeepSeek overlay is ephemeral, has no API key, and supports headless argv', async (t) => {
  ...
  const spec = await windowsLaunch(
    ...
    ['headless', 'hello\nworld'],
    {
      ...
      DEEPSEEK_HARNESS_MAX_TOKENS: '6543',
    },
    { tempRoot: root },
  );
  ...
  assert.equal(
    spec.env.SUBCONSCIOUS_DSH_BASE_URL,
    'https://gateway.example/v1',
  );
...
for (const client of ['codex', 'cursor', 'copilot'])
  test(`${client} hooks merge idempotently and remove only owned entries`, () => {
    const custom = {
      matcher: 'keep',
      extra: true,
      hooks: [
        { type: 'command', command: 'custom-script.exe' },
        { type: 'command', command: 'bash subconscious-hook.sh' },
      ],
    };
    const original = { extra: { keep: true }, hooks: { PreCompact: [custom] } };
    const command = windowsHookCommand(
      client,
      'C:\\User & %name%\\hook.cjs',
      'C:\\config.json',
    );
    ...
    assert.deepEqual(removed.hooks.PreCompact, [
      { ...custom, hooks: [custom.hooks[0]] },
    ]);
...
test('malformed hook configuration is rejected without overwriting any file', async (t) => {
  ...
  await assert.rejects(
    windowsSetup('codex', 'install', [], env, { home, log: quiet }),
    /must be an array/,
  );
  assert.equal(await fs.readFile(file, 'utf8'), before);
  await assert.rejects(
    fs.access(path.join(home, '.codex', 'subconscious-windows.json')),
  );
  await fs.writeFile(file, '{ malformed');
  await assert.rejects(
    windowsSetup('codex', 'uninstall', [], {}, { home, log: quiet }),
    /leaving it unchanged/,
  );
  assert.equal(await fs.readFile(file, 'utf8'), '{ malformed');
});

for (const client of ['codex', 'cursor', 'copilot', 'pi'])
  test(`${client} Windows setup survives install/status/reinstall/uninstall`, async (t) => {
    const home = await temporary(t);
    const appData = path.join(home, 'AppData', 'Roaming');
    ...
    const providerFile =
      client === 'pi'
        ...
        : path.join(appData, 'Code', 'User', 'chatLanguageModels.json');
    if (client === 'pi')
      await writeJson(providerFile, {
        extra: true,
        providers: { existing: { apiKey: 'keep' } },
      });
    if (client === 'copilot')
      await writeJson(providerFile, [{ name: 'Existing', models: [] }]);
    assert.equal(
      await windowsSetup(client, 'install', [], environment, options),
      0,
    );
    assert.equal(
      await windowsSetup(client, 'install', [], environment, options),
    ...
    if (client === 'copilot') {
      ...
      assert.equal(
        providers[1].models[0].url,
        'https://gateway.example/v1/messages',
      );
      assert.equal(
        providers[1].apiKey,
        // biome-ignore lint/suspicious/noTemplateCurlyInString: VS Code secret input, not a JS template
        '${input:chat.lm.secret.subconscious-gateway}',
      );
    ...
    await windowsSetup(client, 'uninstall', [], { APPDATA: appData }, options);
    if (client === 'copilot')
      assert.deepEqual(JSON.parse(await fs.readFile(providerFile, 'utf8')), [
...
test('Copilot uses Roaming AppData and validates app and token options before writing', async (t) => {
  ...
  await windowsSetup(
    'copilot',
    'install',
    ['--vscode-app', 'Code - Insiders', '--max-input-tokens', '900000'],
    environment,
    options,
  );
  ...
  assert.equal(
    JSON.parse(await fs.readFile(file, 'utf8'))[0].models[0].maxInputTokens,
    900000,
  );
});
...
test('legacy cleanup is auth-free and preserves unrelated OpenCode providers and Claude settings', async (t) => {
  ...
  await writeJson(path.join(opencode, 'opencode.json'), {
    provider: { subconscious: {}, other: { keep: true } },
  ...
  assert.deepEqual(
    JSON.parse(await fs.readFile(path.join(opencode, 'opencode.json'), 'utf8')),
    { provider: { other: { keep: true } }, theme: 'dark' },
  );
  assert.deepEqual(
    JSON.parse(await fs.readFile(path.join(claude, 'settings.json'), 'utf8')),
    { theme: 'dark' },
  );
  await assert.rejects(fs.access(path.join(opencode, 'subconscious.env')));
  await assert.rejects(
    fs.access(path.join(claude, 'subconscious-gateway.env')),
  );
});
...
test('Windows hooks report prompt and compaction events without Bash, jq or curl', async (t) => {
  ...
  const base = await gateway(t, (req, res) => {
    ...
    req.on('end', () => {
      requests.push({ body: JSON.parse(text), headers: req.headers });
      res.end('{}');
    });
  });
  ...
  assert.deepEqual(
    await emit('cursor', {
      ...
      conversation_id: 'c1',
      prompt: 'hello & %PATH%',
      workspace_roots: ['C:\\My Projects\\project'],
    }),
  ...
  assert.deepEqual(
    requests.map((r) => r.body.phase || r.body.event),
    [
      'conversation_ensure',
      'point',
      'start',
  ...
  assert.ok(
    requests.every((r) => r.headers.authorization === 'Bearer sk-test'),
  );
  assert.deepEqual(
    await fs.readdir(path.join(home, 'subconscious-compact-pending')),
    [],
  );
});
...
test('Windows upgrades retain the global npm prefix and the TUI chooses .exe', () => {
  const target = detectInstallTarget(
    'C:\\Users\\First Last\\AppData\\Roaming\\npm\\node_modules\\subconscious-cli\\bin\\update-check.js',
    'win32',
  );
  ...
  assert.equal(
    nativeTargetName('win32', 'arm64'),
    'subc-tui-windows-arm64.exe',
  );
});

test('Windows host: real npm/npx shims and PowerShell hook argv run natively', {
  skip: !isWindows,
}, async (t) => {
...
test('Windows host: CLI routes every agent with a Bash-free PATH and preserves exit codes', {
  ...
  await fs.writeFile(
    path.join(bin, 'sc.cmd'),
    '@ECHO off\necho SERVICE_CONTROL_MUST_NOT_RUN\nexit /b 99\n',
  );
  const base = await gateway(t, (_req, res) => {
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ data: [{ id: env.MODEL }] }));
  });
  const environment = windowsEnv(process.env, {
    ...
    SUBCONSCIOUS_BASE_URL: base,
    SUBCONSCIOUS_MODEL: env.MODEL,
    SUBC_TEST_RECORD: record,
  ...
  for (const name of ['cursor', 'copilot', 'pi', 'codex']) {
    const result = await child(process.execPath, [cli, name, 'install'], {
      env: environment,
    });
  ...
  for (const command of [
    ...
    const result = await child(process.execPath, [cli, ...command], {
      env: environment,
    });
