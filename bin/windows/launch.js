...
export async function windowsLaunch(
  ...
  if (id === 'claude-code')
    return { command: 'claude', ...claudeNativeLaunch(argv, env) };
  const keyName = {
    codex: 'CODEX_API_KEY',
    opencode: 'OPENCODE_API_KEY',
    pi: 'PI_API_KEY',
    'subconscious-code': 'SC_API_KEY',
    'deepseek-harness': 'DEEPSEEK_HARNESS_API_KEY',
  }[id];
  const { model, base, key } = connection(env, keyName);
  const childEnv = {
    ...env,
    SUBCONSCIOUS_API_KEY: key,
    SUBCONSCIOUS_GATEWAY_URL: base,
  };
  ...
  if (id === 'opencode') {
    const context = positiveInteger(
      value(env, 'OPENCODE_CONTEXT_LIMIT', '5000000'),
      'OPENCODE_CONTEXT_LIMIT',
    );
    const output = positiveInteger(
      value(env, 'OPENCODE_OUTPUT_LIMIT', '65536'),
    ...
    return {
      command: 'opencode',
      args: parseOptions(argv, {}).rest,
      env: childEnv,
    };
  }
  if (id === 'codex') {
    const { options: opts, rest } = parseOptions(argv, {
      '--context-window': 1,
      '--max-context-window': 1,
      '--auto-compact-token-limit': 1,
      '--reasoning-effort': 1,
      '--subagents': true,
      '--external-tools': true,
    });
    const context = positiveInteger(
      opts['--context-window'] || value(env, 'CODEX_CONTEXT_WINDOW', '5000000'),
      'context-window',
    );
    const maxContext = positiveInteger(
      opts['--max-context-window'] ||
        value(env, 'CODEX_MAX_CONTEXT_WINDOW', String(context)),
      'max-context-window',
    );
    const compact = positiveInteger(
      opts['--auto-compact-token-limit'] ||
        value(env, 'CODEX_AUTO_COMPACT_TOKEN_LIMIT', '4500000'),
      'auto-compact-token-limit',
    );
    const reasoning =
      opts['--reasoning-effort'] || value(env, 'CODEX_REASONING_EFFORT', 'max');
    if (!['none', 'low', 'medium', 'high', 'max'].includes(reasoning))
      throw new Error(
    ...
    const threads = positiveInteger(
      value(env, 'MAX_CONCURRENT_SUBAGENTS', '4'),
    ...
    try {
      ...
      if (
        !opts['--external-tools'] &&
        value(env, 'CODEX_EXTERNAL_TOOLS', 'false') !== 'true'
      ) {
      ...
      return {
        command: opts['--subagents'] ? 'npx' : 'codex',
        args: [
          ...(opts['--subagents'] ? ['-y', '@openai/codex@0.132.0'] : []),
          ...args,
          ...rest,
        ],
        env: childEnv,
  ...
  if (id === 'deepseek-harness') {
    const context = positiveInteger(
      value(env, 'DEEPSEEK_HARNESS_CONTEXT_WINDOW', '5000000'),
      'DEEPSEEK_HARNESS_CONTEXT_WINDOW',
    );
    const maxTokens = positiveInteger(
      value(env, 'DEEPSEEK_HARNESS_MAX_TOKENS', '65536'),
