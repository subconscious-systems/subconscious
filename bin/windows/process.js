  ...
  return [
    ...new Set(
      [
        ...
        env.LOCALAPPDATA &&
          path.win32.join(env.LOCALAPPDATA, 'Microsoft', 'WinGet', 'Links'),
        env.SYSTEMROOT &&
          path.win32.join(
            env.SYSTEMROOT,
            'System32',
...
export function windowsPath(extraDirs, currentPath = '') {
  const seen = new Set();
  return [...extraDirs, ...currentPath.split(';')]
    .filter((dir) => {
      const key = dir
        .replace(/^"|"$/g, '')
        .replace(/[\\/]+$/, '')
...
export function resolveWindowsExecutable(command, options = {}) {
  ...
  const extensions = [
    ...new Set([
      '.exe',
      '.com',
      ...(env.PATHEXT || '.CMD;.BAT').split(';').map((x) => x.toLowerCase()),
      '',
    ]),
  ];
  const hasExtension = extensions.some(
    (ext) => ext && command.toLowerCase().endsWith(ext),
  );
  ...
  for (const dir of dirs) {
    for (const ext of hasExtension ? [''] : extensions) {
      const candidate = dir
        ? pathApi.join(dir.replace(/^"|"$/g, ''), command + ext)
        : command + ext;
...
export function windowsInvocation(
  command,
  args = [],
  ...
  if (!executable)
    throw new Error(`Could not find ${command} on the Windows PATH.`);
  if (!/\.(cmd|bat)$/i.test(executable))
    return { command: executable, args, env };
  const readFile =
    options.readFile || ((file) => fs.readFileSync(file, 'utf8'));
  const pathApi = options.pathApi || path.win32;
  const contents = readFile(executable);
  const candidates = [
    ...contents.matchAll(/%(?:dp0%|~dp0)[\\/]?([^"\r\n]+)"/gi),
  ];
  const name = pathApi
    .basename(executable)
    .replace(/\.(cmd|bat)$/i, '')
  ...
  if (/ELECTRON_RUN_AS_NODE\s*=\s*1/i.test(contents)) {
    electron = candidates
      .map((match) => pathApi.resolve(pathApi.dirname(executable), match[1]))
      .find(
        (target) =>
          /\.exe$/i.test(target) &&
  ...
  for (const match of candidates) {
    ...
    if (/\.(exe|com)$/i.test(target)) {
      if (!electron && !/node\.exe$/i.test(target) && fs.existsSync(target))
        return { command: target, args, env };
      continue;
    }
    if (
      ['npm', 'npx'].includes(name) &&
      pathApi.basename(target) !== `${name}-cli.js`
    )
      continue;
    let source;
    try {
  ...
  throw new Error(
    `Cannot safely launch the custom batch wrapper ${executable}. Install the native executable or standard npm package for ${command}.`,
  );
}
...
export function openWindowsBrowser(url) {
  const invocation = powershellCommand(
    'Start-Process -FilePath $env:SUBC_BROWSER_URL',
    { ...process.env, SUBC_BROWSER_URL: url },
  );
  return runWindows(invocation.command, invocation.args, {
    env: invocation.env,
    stdio: 'ignore',
  });
}
...
export function windowsEditor(requested, environment = process.env) {
  const env = windowsEnv(environment);
  const text =
    requested || env.VISUAL?.trim() || env.EDITOR?.trim() || 'notepad.exe';
  ...
  const parts = fs.existsSync(text)
    ? [text]
    : text.match(/"[^"]*"|[^\s"]+/g) || [];
  if ((text.match(/"/g) || []).length % 2)
    throw new Error('Unmatched quote in VISUAL / EDITOR');
  const [command, ...args] = parts.map((part) => part.replace(/^"|"$/g, ''));
  if (!command) throw new Error('Editor is empty');
  if (
    /^(code|code-insiders)(\.cmd|\.exe)?$/i.test(
