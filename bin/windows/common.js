...
export function parseOptions(argv, schema, { strict = false } = {}) {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--') {
      if (strict && i + 1 < argv.length)
        throw new Error(`Unexpected setup argument: ${argv[i + 1]}`);
...
export async function readJson(file, fallback) {
  try {
    ...
    throw new Error(
      `Cannot read ${file}; leaving it unchanged: ${error.message}`,
    );
  }
}
export function object(value, description) {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error(
      `${description} must be a JSON object; leaving it unchanged`,
    );
  return value;
}
