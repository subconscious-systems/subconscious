...
export function windowsHookCommand(
  ...
  const data = Buffer.from(
    JSON.stringify({ client, script, config, node }),
  ).toString('base64');
...
export function mergeHooks(document, client, command) {
  ...
  for (const [event, entries] of Object.entries(hooks)) {
    if (!Array.isArray(entries))
      throw new Error(`hooks.${event} must be an array; leaving it unchanged`);
    const kept = [];
    for (const entry of entries) {
      ...
      if (Array.isArray(entry.hooks)) {
        ...
        if (filtered.length || entry.hooks.length === 0)
          kept.push({ ...entry, hooks: filtered });
      } else if (entry.hooks !== undefined) {
        throw new Error(
          `hooks.${event}.hooks must be an array; leaving it unchanged`,
        );
  ...
  if (command) {
    const events = {
      ...
      cursor: ['beforeSubmitPrompt', 'preCompact'],
      copilot: ['UserPromptSubmit', 'PreCompact'],
    }[client];
    for (const event of events) {
      const hook = {
        type: 'command',
        command,
        timeout: client === 'copilot' ? 5 : 3,
      };
      const entry =
        client === 'codex'
          ? {
              hooks: [
                { ...hook, statusMessage: 'Reporting Subconscious compaction' },
              ],
            }
          : hook;
...
export async function windowsSetup(
  ...
  if (installing && needsKey && !apiKey)
    throw new Error('An API key is required for setup. Run subc login first.');

  if (['codex', 'cursor', 'copilot'].includes(id)) {
    const dir =
      id === 'codex'
        ? env.CODEX_DIR || env.CODEX_HOME || path.join(home, '.codex')
        : path.join(home, `.${id}`);
    const hooksFile =
      id === 'copilot'
        ? path.join(dir, 'hooks', 'subconscious-hooks.json')
    ...
    const merged = mergeHooks(
      document,
      id,
      installing ? windowsHookCommand(id, script, config) : null,
    );
    ...
    if (id === 'copilot') {
      ...
      if (app && !apps.includes(app))
        throw new Error(
          '--vscode-app must be Code, Code - Insiders, or VSCodium',
        );
      if (!app) {
        for (const candidate of apps)
          if (await exists(path.join(appData, candidate, 'User'))) {
            app = candidate;
            break;
          }
        app ||= 'Code';
      ...
      if (!Array.isArray(current))
        throw new Error(
          `${providerFile} must be a JSON array; leaving it unchanged`,
        );
      for (const provider of current)
        object(provider, 'language model provider');
    ...
    if (action === 'status') {
      const hasHook = Object.values(document.hooks || {}).some((entries) =>
        entries.some(
          (entry) =>
            owned(entry.command) ||
            entry.hooks?.some((hook) => owned(hook.command)),
        ),
      );
      log(
        `${id}: ${hasHook ? 'Subconscious hooks installed' : 'no Subconscious hooks'} (${hooksFile})`,
      );
    ...
    if (installing || (await exists(hooksFile)))
      await writeJson(hooksFile, merged);
    if (providerFile && (installing || (await exists(providerFile))))
      await writeJson(providerFile, providers);
    if (removing) {
      await fs.rm(script, { force: true });
      await fs.rm(config, { force: true });
    }
    if (installing && id === 'cursor')
      log(
        `In Cursor Settings, use ${gatewayUrl}/v1, your API key, and model ${value(env, 'MODEL', defaults.model)}. Restart Cursor to load hooks.`,
      );
    if (installing && id === 'copilot')
      log(
        'Restart VS Code, select a Subconscious model, and enter your API key when prompted.',
      );
  ...
  if (id === 'pi') {
    ...
    if (action === 'status') {
      log(
        `Pi: ${providers.subconscious ? 'Subconscious provider installed' : 'not installed'} (${modelsFile})`,
      );
      return 0;
    }
    ...
    if (installing || (await exists(modelsFile)))
      await writeJson(modelsFile, { ...document, providers });
  ...
  if (id === 'claude-code' || id === 'opencode') {
    ...
    const legacy = path.join(
      dir,
      id === 'claude-code' ? 'subconscious-gateway.env' : 'subconscious.env',
    );
    ...
    log(
      `${id}: ${removing ? 'legacy integration removed' : present ? 'legacy integration exists' : 'launch-only; no legacy integration'}.`,
    );
    return 0;
  }
